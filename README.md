...
- 🧠 **Multi-Model AI Engine** — GPT-4o, Claude, Gemini, or local Ollama VLMs (like `qwen3-vl:2b-instruct`)
...
- 📁 **Smart Inputs** — Handles file uploads, calendars, dropdowns, hover, scroll, and keypresses
...
|---|---|---|---|---|
| **Ollama** (local) | `qwen3-vl:2b-instruct`, any VLM | 🆓 Free | ⚡ Fastest | 🔒 100% local |
...
> 💡 **Want zero API costs?** Use Ollama with a local VLM. Install Ollama, pull a vision model (`ollama pull qwen3-vl:2b-instruct`), and set `provider: "ollama"` in your config. All processing stays on your machine.
...
1. **Actor**: Captures the viewport as a JPEG buffer → sends to the VLM → receives action coordinates
...
| **Heavy Asset Blocking** | Blocks images, fonts, and analytics for faster page loads |
...
Vouch works in headless CI environments:
...
- name: Run E2E tests
  env:
    ...
    VOUCH_MODEL: gpt-4o
    VOUCH_HEADLESS: true
  run: vouch run tests/checkout.vch
```
